  ...
  <div id="operationalDeployment">
    <header>
      ...
      <Select v-model="scenario" style="width:200px" @on-change="addLayer">
        <Option v-for="item in scenarios" :value="item.value" :key="item.value">{{ item.label }}</Option>
      </Select>
      <!-- <span>制作</span>
    ...
    <div class="content">
      <div class="left demo-tabs-style1">
        <Tabs type="card">
          <TabPane label="编成">
            <Tree :data="combatCompilation_data" :render="renderContent" class="demo-tree-render"></Tree>
          </TabPane>
          <TabPane label="态势">
            <Tree :data="combatTaishi_data" :render="renderContent" class="demo-tree-render"></Tree>
          </TabPane>
          <TabPane label="编组">
            <Tree :data="combatGroup_data" :render="renderContent" class="demo-tree-render"></Tree>
          </TabPane>
        </Tabs>
      </div>
      <div id="map2d"></div>
...
export default {
  ...
  methods: {
    ...
    setToCup(val,ponit){
      console.log(val,ponit)
      if(ws&&socketOpen){
        let _center = ol.proj.transform(ponit, 'EPSG:3857', 'EPSG:4326')
        let _name = "";
        if(val == "20407"){//导弹发射
          _name = "导弹发射车"
        }else if(val == "2040101"){//轻型坦克
          _name = "坦克1"
        }else if(val == "2040104"){ //侦察坦克
          _name = "坦克2"
        }else if(val == "20401"){//水路坦克
          _name = "坦克3"
        }else if(val == "2040103"){//轮式坦克
          _name = "坦克4"
        }
        let message = {
          name:_name,
    ...
    addLayer(val) {
      if(val == "印巴想定1"){
        ...
        let extent= [63.824090,25.891786,88.657235,36.055921]
      ...
      this.combatGroup_data = operationalDeployment_data.combatGroup_data;
        ...
        this.map.DrawPlot.callback = this.setToCup
      }else if(val == "泄露想定1"){
        let url = "./static/img/XL1.png"
        let extent= [118.686268,34.094136,119.057784,34.263259]
        this.map.addImageLayer("map2d",url,extent);
        let _center = ol.proj.transform([118.865793,34.186787], 'EPSG:4326', 'EPSG:3857')
